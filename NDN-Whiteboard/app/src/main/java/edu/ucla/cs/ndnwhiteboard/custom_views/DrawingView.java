...
import edu.ucla.cs.ndnwhiteboard.tasks.WhiteboardListener;
...
public class DrawingView extends View {
    ...
    public WhiteboardListener whiteboardListener;
    ...
    public void setWhiteboardActivity(WhiteboardActivity whiteboardActivity) {
        ...
        whiteboardListener = new WhiteboardListener(whiteboardActivity);
    }
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        switch (event.getAction()) {
            ...
            case MotionEvent.ACTION_UP:
                ...
                String jsonString = jsonObject.toString();
                whiteboardListener.informUsers(jsonString);
                history.add(jsonString);
                whiteboardActivity.callback(jsonString);
                break;
        ...
        try {
            ...
            try {
                ...
                if (type.equals("pen") || type.equals("eraser")) {
                    ...
                    float touchX = (float)Double.parseDouble(startPoint.getString(0).replace("\"", "").replace(",", ".")) * viewWidth;//(float) startPoint.getDouble(0) * viewWidth;
                    float touchY = (float)Double.parseDouble(startPoint.getString(1).replace("\"", "").replace(",", ".")) * viewWidth;//(float) startPoint.getDouble(1) * viewWidth;
                    drawPath.moveTo(touchX, touchY);
                    for (int i = 1; i < coordinates.length(); i++) {
                        JSONArray point = coordinates.getJSONArray(i);
                        float x = (float)Double.parseDouble(point.getString(0).replace("\"", "").replace(",", ".")) * viewWidth;
                        float y = (float)Double.parseDouble(point.getString(1).replace("\"", "").replace(",", ".")) * viewWidth;//(float) point.getDouble(1) * viewWidth;
                        Log.e(TAG, "Drawing " + x + " " + y);
                        drawPath.lineTo(x, y);
                    }
                    drawCanvas.drawPath(drawPath, drawPaint);
                    invalidate();
