...
import edu.ucla.cs.ndnwhiteboard.IntroActivity;
...
public class PingTask extends AsyncTask<Void, Void, String> {
    ...
    protected String doInBackground(Void... voids) {
        Log.d(TAG, "Ping Task (doInBackground); hostname: " + IntroActivity.hostName);
        try {
            // Initialize the Face
            ndnActivity.m_face = new Face(IntroActivity.hostName);

            // Express the ping Interest
            ndnActivity.m_face.expressInterest(new Name("/localhop/nfd/rib/list"),
                    new OnData() {
                        @Override
                        public void
